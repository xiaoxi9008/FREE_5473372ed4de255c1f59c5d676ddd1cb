...
function Library:AddSnowEffect(Parent: GuiObject, SnowCount, SnowSize, Speed, Color)
...
local BackgroundContainer = New("Frame", {
    ...
    Library:AddSnowEffect(BackgroundContainer, 40, 10, 0.7)
...
function Library:CreateLoading(LoadingInfo)
    ...
    --// 边框 - 使用 UIStroke \\--
    local MainOutline = New("UIStroke", {
        Color = "OutlineColor",
        Thickness = 1.5,
        Transparency = 0.3,
        Parent = MainFrame,
    ...
    --// 侧边栏边框 \\--
    local SideOutline = New("UIStroke", {
        Color = "OutlineColor",
        Thickness = 1,
        Transparency = 0.2,
        Parent = SideBar,
    })
    local SideShadow = New("UIStroke", {
        Color = "DarkColor",
        Thickness = 2,
        Transparency = 0.3,
        Parent = SideBar,
    ...
    if LoadingInfo.Icon then
        ...
        PulseTween = TweenService:Create(WindowIcon, 
    ...
    local MiddleRingStroke = New("UIStroke", {
        Color = "OutlineColor",
        Thickness = 1.5,
        Transparency = 0.4,
        ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
    ...
    local MiddleRingTween = TweenService:Create(MiddleRing, 
        TweenInfo.new(3, Enum.EasingStyle.Linear, Enum.EasingDirection.Out, -1), 
        { Rotation = -360 }
    )
    MiddleRingTween:Play()
    ...
    for i = 1, LoadingInfo.TotalSteps do
        ...
        local DotGlow = New("Frame", {
            AnchorPoint = Vector2.new(0.5, 0.5),
            BackgroundColor3 = "AccentColor",
            BackgroundTransparency = i <= LoadingInfo.CurrentStep and 0.8 or 1,
            Position = UDim2.fromScale(0.5, 0.5),
            Size = UDim2.fromOffset(14, 14),
            Parent = DotContainer,
        })
        ...
        table.insert(StepDots, { Dot = Dot, Glow = DotGlow })
    end
    ...
    function Loading:SetMessage(Text: string)
    ...
    function Loading:SetDescription(Text: string)
    ...
    function Loading:SetLoadingIcon(Icon: string)
    ...
    function Loading:SetLoadingIconTweenTime(TweenTime: number)
    ...
    function Loading:SetLoadingIconColor(Color: Color3)
        LoadingIcon.ImageColor3 = Color
    end

    function Loading:SetCurrentStep(Step: number)
        Loading.CurrentStep = math.clamp(Step, 0, Loading.TotalSteps)

        local Progress = Loading.CurrentStep / Loading.TotalSteps
        TweenService:Create(SliderFill, TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { 
        ...
        for i, DotData in ipairs(StepDots) do
            local Active = i <= Loading.CurrentStep
            TweenService:Create(DotData.Dot, Library.TweenInfo, {
                BackgroundColor3 = Active and Library.Scheme.AccentColor or Library.Scheme.MainColor,
                Size = Active and UDim2.fromOffset(8, 8) or UDim2.fromOffset(6, 6),
            }):Play()
            TweenService:Create(DotData.Glow, Library.TweenInfo, {
                BackgroundTransparency = Active and 0.8 or 1,
                Size = Active and UDim2.fromOffset(16, 16) or UDim2.fromOffset(14, 14),
            }):Play()
        end
    end

    function Loading:SetTotalSteps(Steps: number)
        Loading.TotalSteps = Steps
        
        for _, DotData in StepDots do
            DotData.Dot:Destroy()
            DotData.Glow:Destroy()
        end
        ...
        for i = 1, Steps do
            ...
            local DotGlow = New("Frame", {
                AnchorPoint = Vector2.new(0.5, 0.5),
                BackgroundColor3 = "AccentColor",
                BackgroundTransparency = i <= Loading.CurrentStep and 0.8 or 1,
                Position = UDim2.fromScale(0.5, 0.5),
                Size = UDim2.fromOffset(14, 14),
                Parent = DotContainer,
            })
    ...
    function Loading:SetWindowHeight(Height: number)
        Loading.WindowHeight = Height
        Loading:UpdateLayout()
    end

    function Loading:SetWindowWidth(Width: number)
        Loading.WindowWidth = Width
        Loading:UpdateLayout()
    end

    function Loading:SetContentWidth(Width: number)
        Loading.ContentWidth = Width
        Loading:UpdateLayout()
    end

    function Loading:SetSidebarWidth(Width: number)
        Loading.SidebarWidth = Width
        Loading:UpdateLayout()
    end

    function Loading:ShowSidebarPage(Bool: boolean)
        Loading.ShowSidebar = Bool
        Loading:UpdateLayout()
    end

    function Loading:ShowErrorPage(Enabled: boolean)
    ...
    function Loading:RecalculateErrorHeight()
        local TargetWidth = (Loading.ShowSidebar and Loading.ContentWidth or Loading.WindowWidth) - 60
        local _, ErrorY = Library:GetTextBounds(ErrorLabel.Text, Library.Scheme.Font, 14, TargetWidth)
        ErrorLabel.Size = UDim2.new(1, -60, 0, ErrorY + 4)
        ...
        local RequiredHeight = 
            ...
            28 + -- Title Height
            8 + -- Title to Label
            ErrorY + 4 + -- Label Height
            16 + -- Label to Buttons
            (HasButtons and 44 or 0) + -- Buttons Area
            24 -- Bottom Padding

        Loading.WindowErrorHeight = RequiredHeight
    end

    function Loading:SetErrorMessage(Text: string)
    ...
    function Loading:SetErrorButtons(Buttons: {})
        ...
        local HasButtons = next(Buttons) ~= nil
        ...
        for Idx, ButtonInfo in Buttons do
            ...
            local BtnLabel = New("TextLabel", {
                BackgroundTransparency = 1,
                Size = UDim2.fromScale(1, 1),
                Text = ButtonInfo.Title or tostring(Idx),
                TextColor3 = TextColor,
                TextSize = 14,
                Parent = TextBtn,
            })
            ...
            local LabelX, _ = Library:GetTextBounds(BtnLabel.Text, Library.Scheme.Font, 14, 250)
            ButtonContainer.Size = UDim2.fromOffset(LabelX + 40, 32)
            TextBtn.Size = UDim2.fromOffset(LabelX + 40, 32)
    ...
    function Loading:Destroy()
        ...
        if MiddleRingTween then
            MiddleRingTween:Cancel()
        end
        if PulseTween then
            PulseTween:Cancel()
        end
